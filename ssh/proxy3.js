...
server.on('connection', function(socket) {
    ...
    var htmlMessage = fs.readFileSync('/etc/proxy-message.html', 'utf8');
    socket.write("HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nContent-Length: " + htmlMessage.length + "\r\n\r\n" + htmlMessage, function(err) {
        if(err) {
            console.log("[SWRITE] Failed to write response to " + socket.remoteAddress + ":" + socket.remotePort + ", error: " + err);
        } else {
            console.log("[INFO] Sent 200 OK response with styled HTML to " + socket.remoteAddress + ":" + socket.remotePort);
        }
    });
