...
if [[ $ssh_udp_service == "running" ]]; then 
   status_ssh_udp=" ${GREEN}Running ${NC}( No Error )"
else
   status_ssh_udp="${RED}  Not Running ${NC}  ( Error )"
fi
...
echo -e "     ${BIWhite}»${NC} ${BICyan}SSH UDP                 :${NC} $status_ssh_udp      ${NC}"
